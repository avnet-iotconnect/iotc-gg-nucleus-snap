...
# Prevent duplicate Greengrass instances (same root)
if pgrep -f "Greengrass.jar" >/dev/null; then
    for pid in $(pgrep -f "Greengrass.jar"); do
        if ps -p "$pid" -o args= | grep -q "-Droot=$GREENGRASS_DIR"; then
            echo "Greengrass already running with root $GREENGRASS_DIR (PID: $pid). Exiting."
            exit 0
        fi
    done
fi
...
if [ ! -f "$JAVA_BIN" ]; then
    ...
    JAVA_BIN="$(find "$SNAP/usr/lib/jvm" -path "*/bin/java" -type f 2>/dev/null | head -n 1)"
    if [ -z "$JAVA_BIN" ]; then
        find "$SNAP" -name "java" -type f 2>/dev/null || echo "No Java binary found in snap"
        exit 1
    fi
    echo "Using fallback Java binary: $JAVA_BIN"
fi
