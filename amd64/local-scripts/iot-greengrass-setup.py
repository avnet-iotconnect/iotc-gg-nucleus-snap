...
DEFAULT_KIT_DIRNAME = "connection-kit"

def parse_args():
    parser = argparse.ArgumentParser(description="AWS IoT Greengrass setup")
    parser.add_argument(
        "--connection-kit",
        help="Path to an IOTCONNECT Greengrass connection kit zip",
    )
    parser.add_argument(
        "--kit-dir",
        help="Optional extraction directory for the kit",
    )
    return parser.parse_args()

def extract_connection_kit(zip_path, dest_dir):
    if not os.path.exists(zip_path):
        print(f"Error: connection kit not found: {zip_path}")
        return None
    ...
    try:
        ...
        print(f"Error extracting connection kit: {e}")
        return None

def resolve_kit_paths(config, kit_dir, greengrass_root):
    system = config.get("system", {})
    cert_path = os.path.join(kit_dir, "device.pem.crt")
    key_path = os.path.join(kit_dir, "private.pem.key")
    root_ca_path = os.path.join(kit_dir, "AmazonRootCA1.pem")

    system["privateKeyPath"] = key_path
    system["certificateFilePath"] = cert_path
    system["rootCaPath"] = root_ca_path
    system.setdefault("rootPath", greengrass_root)
    config["system"] = system

    return config

def get_nucleus_service(config):
    services = config.get("services", {})
    if "aws.greengrass.Nucleus" in services:
        return "aws.greengrass.Nucleus"
    if "aws.greengrass.NucleusLite" in services:
        return "aws.greengrass.NucleusLite"
    return None

def setup_from_connection_kit(zip_path, kit_dir_override=None):
    print("=== Connection Kit Setup ===")

    snap_common = os.environ.get('SNAP_COMMON', '/tmp')
    greengrass_root = f"{snap_common}/greengrass/v2"
    kit_dir = kit_dir_override or f"{greengrass_root}/{DEFAULT_KIT_DIRNAME}"

    extracted_dir = extract_connection_kit(zip_path, kit_dir)
    if not extracted_dir:
        return False

    config_path = os.path.join(extracted_dir, "config.yaml")
    if not os.path.exists(config_path):
        print(f"Error: config.yaml not found in kit: {extracted_dir}")
        return False
    ...
    try:
        ...
        print(f"Error reading kit config: {e}")
        return False

    config = resolve_kit_paths(config, extracted_dir, greengrass_root)

    os.makedirs(greengrass_root, exist_ok=True)
    resolved_config_path = os.path.join(greengrass_root, "config.yaml")
    try:
        with open(resolved_config_path, 'w') as f:
            yaml.dump(config, f, default_flow_style=False)
    except Exception as e:
        print(f"Error writing resolved config: {e}")
        return False

    nucleus_service = get_nucleus_service(config)
    if not nucleus_service:
        print("Error: Nucleus service not found in config.yaml")
        return False

    system = config.get("system", {})
    thing_name = system.get("thingName", "GreengrassCore")
    cert_path = system.get("certificateFilePath")
    key_path = system.get("privateKeyPath")
    root_ca_path = system.get("rootCaPath")

    svc_cfg = config["services"][nucleus_service].get("configuration", {})
    region = svc_cfg.get("awsRegion")
    iot_data_endpoint = svc_cfg.get("iotDataEndpoint")
    iot_cred_endpoint = svc_cfg.get("iotCredEndpoint")
    iot_core_endpoint = iot_data_endpoint

    return install_greengrass_v2(
        thing_name,
        region,
        cert_path,
        key_path,
        ...
        iot_data_endpoint,
        iot_cred_endpoint,
        config_override_path=resolved_config_path,
    )
...
def install_greengrass_v2(thing_name, region, cert_path, private_key_path, root_ca_path, 
    ...
    possible_java_paths = []
    if snap_arch:
        possible_java_paths.append(
            f"{snap_dir}/usr/lib/jvm/java-11-openjdk-{snap_arch}/bin/java"
        )
    possible_java_paths.extend([
    ...
    try:
        ...
        if os.path.exists(nucleus_jar):
            ...
            print("✓ Greengrass will be started by the snap daemon")
            print("✓ Do not start Greengrass manually; the snap daemon manages the service")
...
def main():
    ...
    args = parse_args()
    if args.connection_kit:
        success = setup_from_connection_kit(args.connection_kit, args.kit_dir)
        if success:
            print("\n" + "=" * 50)
            print("✓ Greengrass setup from connection kit completed successfully!")
            print("=" * 50)
            sys.exit(0)
        print("\n⚠ Setup from connection kit failed. Check the logs above.")
        sys.exit(1)
